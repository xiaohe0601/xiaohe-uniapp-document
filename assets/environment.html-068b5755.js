import{_ as e,W as n,X as i,a1 as d}from"./framework-b90dbfae.js";const a={},l=d(`<div class="hint-container tip"><p class="hint-container-title">提示</p><p>由于项目采用的是HBuilderX创建方式，默认不支持类似于Vue cli创建方式的env环境变量，故项目中模仿Vue cli使用 <code>dotenv</code> 实现了env环境变量，具体实现可查看 <code>vue.config.js</code>，也可根据实际需要做对应修改</p></div><div class="hint-container info"><p class="hint-container-title">相关信息</p><ul><li><p>之所以不采用Vue cli创建项目，是因为HBuilderX对cli项目的支持并不友好，例如不能自动跳转文件对应页面，App在线打包时基础库更新麻烦等</p></li><li><p>同时因为HBuilderX创建的项目使用的编译器是集成在HBuilderX中的，所以当HBuilderX有新版本时请及时更新，以免造成一些未知的异常</p></li></ul></div><div class="hint-container warning"><p class="hint-container-title">注意</p><p>虽然是模仿Vue cli的环境变量，但是项目中对其做出了一些优化以及差异化，例如支持了除字符串以外的其他数据类型、修改了变量前缀等，具体使用规范如下</p></div><h3 id="命名规范" tabindex="-1"><a class="header-anchor" href="#命名规范" aria-hidden="true">#</a> 命名规范</h3><ol><li><p>key：全大写下划线命名，并且以<code>UNI_APP_</code>开头（<code>ENVIRONMENT</code>除外）</p><div class="language-env line-numbers-mode" data-ext="env"><pre class="language-env"><code>UNI_APP_EXAMPLE
